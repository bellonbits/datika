...
export default function AdminCourseSettingsPage() {
  ...
  const deleteCourse = async () => {
    if (!confirm('ADMIN OVERRIDE: Are you sure you want to permanently delete this course from the platform? This cannot be undone.')) return;
    try {
  ...
  if (isLoading) return (
     <div className="h-full flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
           ...
           <span className="text-white/20 text-xs font-black uppercase tracking-widest">Enforcing Settings...</span>
        </div>
     </div>
  );
    ...
    <div className="p-6 h-full overflow-auto text-white" style={{ fontFamily: 'Inter, system-ui, sans-serif' }}>
      ...
      <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="flex items-center justify-between gap-6 mb-10">
        <div className="flex items-center gap-5">
           ...
           <div>
              <h1 className="text-2xl font-black mb-1 text-purple-400">Platform Settings</h1>
              <p className="text-white/30 text-xs font-semibold">Admin-level metadata control</p>
           </div>
        </div>
        ...
        <button 
          ...
          disabled={isSaving}
          className="h-11 px-8 rounded-2xl bg-orange-500 hover:bg-orange-400 text-[#070b16] text-xs font-black uppercase tracking-widest transition-all shadow-xl shadow-orange-500/20 flex items-center gap-2 disabled:opacity-50"
        >
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        <div className="flex items-center gap-2 text-orange-400/50 text-[10px] uppercase font-black tracking-[0.2em] mb-4 pl-2">
        ...
        {success && (
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-xs font-black flex items-center gap-2 mb-4">
             <CheckCircle2 size={16} /> Admin override manifested successfully!
          </motion.div>
        )}

        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6 pb-20">
             {/* General Info */}
             <div className="p-8 border border-white/5 hover:border-orange-500/10 transition-colors" style={glassStyle}>
                <h3 className="text-xs font-black text-white/30 uppercase tracking-[0.2em] mb-8">Platform Identity</h3>
                <div className="space-y-6">
                   <div className="space-y-2">
                     <label className="text-[10px] font-black text-white/20 uppercase tracking-widest px-1">Global Title</label>
                   ...
                   <div className="space-y-2">
                     <label className="text-[10px] font-black text-white/20 uppercase tracking-widest px-1">Marketing Description</label>
                     <textarea
             ...
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Categorization */}
                <div className="p-8 border border-white/5 hover:border-orange-500/10 transition-colors" style={glassStyle}>
                   <h3 className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em] mb-8">Classification Engine</h3>
                   <div className="space-y-6">
                      <div className="space-y-2">
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-white/20 uppercase tracking-widest px-1">Skill Threshold</label>
                ...
                {/* Logistics */}
                <div className="p-8 border border-white/5 hover:border-orange-500/10 transition-colors" style={glassStyle}>
                   <h3 className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em] mb-8">Commercials</h3>
                   <div className="space-y-6">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-white/20 uppercase tracking-widest px-1">Standard Duration</label>
                        <div className="relative">
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-white/20 uppercase tracking-widest px-1">Global Price (KES)</label>
                        <div className="relative">
             ...
             {/* Danger Zone */}
             <div className="p-8 border border-red-500/20 rounded-[32px] bg-red-500/[0.05]">
                <div className="flex items-center gap-3 text-red-500 mb-6 font-black uppercase tracking-[0.3em] text-[10px]">
                   <AlertCircle size={16} /> Executive Override
                </div>
                <div className="flex items-center justify-between">
                   <div>
                      <h4 className="text-sm font-black text-white/90 mb-1">Permanently Delete from Platform</h4>
                      <p className="text-white/40 text-xs font-medium">Warning: This bypasses instructor protections and deletes everything immediately.</p>
                   </div>
                   <button 
                     type="button"
                     onClick={deleteCourse}
                     className="h-12 px-8 rounded-2xl bg-red-500/20 hover:bg-red-500 text-red-500 hover:text-white text-xs font-black transition-all border border-red-500/30"
                   >
                     EXECUTE PURGE
                   </button>
                </div>
