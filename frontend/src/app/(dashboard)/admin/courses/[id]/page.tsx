  ...
  if (isLoading) return (
     <div className="h-full flex items-center justify-center">
        <div className="flex flex-col items-center gap-4">
           ...
           <span className="text-white/20 text-xs font-black uppercase tracking-widest">Accessing Platform Authority...</span>
        </div>
     </div>
  );
        ...
        <div className="flex items-center gap-3">
           ...
           {course.status !== 'PUBLISHED' ? (
              <button 
                ...
                className="h-10 px-6 rounded-xl bg-orange-500 hover:bg-orange-400 text-[#070b16] text-xs font-black uppercase tracking-widest transition-all shadow-xl shadow-orange-500/20 flex items-center gap-2"
              >
                Force Publish (Admin)
              </button>
           ) : (
        ...
        <div className="lg:col-span-3 space-y-6">
           <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {[
                ...
                { label: 'Platform Ranking', value: 'Top 5%', icon: TrendingUp, color: 'text-orange-400', bg: 'bg-orange-400/10' },
                { label: 'Platform Revenue', value: `KES ${course.price * (course._count?.enrollments || 0)}`, icon: BarChart3, color: 'text-emerald-400', bg: 'bg-emerald-400/10' },
              ].map((stat, i) => (
                <motion.div 
                  key={i}
                ...
                >
                   <div className="flex items-center justify-between mb-4">
                      ...
                      <div className="text-[10px] text-white/20 font-black uppercase tracking-tighter">Global</div>
                   </div>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <motion.div 
                ...
                onClick={() => router.push(`/admin/courses/${id}/curriculum`)}
                className="p-8 cursor-pointer hover:border-orange-500/30 transition-all active:scale-[0.98] group"
                style={glassStyle}
              >
                  <div className="w-12 h-12 rounded-2xl bg-orange-500/10 text-orange-400 flex items-center justify-center mb-6 group-hover:bg-orange-500 group-hover:text-[#070b16] transition-all">
                    <BookOpen size={24} />
                  </div>
                  <h3 className="text-xl font-black mb-2">Platform Curriculum</h3>
                  <p className="text-white/30 text-xs font-medium leading-relaxed mb-6">Modify sections, lessons, and assignments. Manage the educational structure directly as an admin.</p>
              ...
              <motion.div 
                ...
                onClick={() => router.push(`/admin/courses/${id}/settings`)}
              ...
              >
                  ...
                  <h3 className="text-xl font-black mb-2">Platform Settings</h3>
                  <p className="text-white/30 text-xs font-medium leading-relaxed mb-6">Enforce metadata rules. Adjust pricing overrides, categorization scaling, and platform-wide visibility.</p>
        ...
        <div className="lg:col-span-1 space-y-6">
           <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="p-6" style={glassStyle}>
              <h4 className="text-[10px] font-black text-white/30 uppercase tracking-[0.2em] mb-6 flex items-center gap-2">
                <Shield size={14} className="text-orange-500/50" /> Admin Insight
              </h4>
              <div className="space-y-4">
                 <div className="flex items-center justify-between">
                    <span className="text-xs text-white/40 font-medium">Author</span>
                    <span className="text-xs font-bold text-orange-400 line-clamp-1">{course.instructor?.name || 'Platform (You)'}</span>
                 </div>
                 <div className="flex items-center justify-between">
                 ...
                 <div className="flex items-center justify-between">
                    ...
                    <span className="text-xs font-bold text-emerald-400">KES {course.price}</span>
                 </div>
              </div>
           </motion.div>
        </div>
